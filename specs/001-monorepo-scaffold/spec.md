...
1. **Given** 一个空的 Git 仓库, **When** 项目初始化完成, **Then** 应包含标准的 monorepo 目录结构（apps/web、apps/server、packages 等）
...
2. **Given** 前端应用配置完成, **When** 查看依赖, **Then** 应包含 Vue 3、Vite、TypeScript、Element-plus 等核心依赖
...
1. **Given** 后端应用目录存在, **When** 运行开发命令, **Then** 应启动开发服务器并监听指定端口
...
- 当开发者使用不同版本的 Node.js 时，如何提示并保证环境一致性？
- 当阿里云镜像源临时不可用时，如何处理？
- 当前后端端口冲突时，如何解决？
...
- **FR-003**: 系统 MUST 包含后端应用目录 (apps/server)，使用 Node.js + TypeScript 技术栈
...
- **Shared Packages**: 可选的共享代码包目录，用于跨应用复用代码
...
- **SC-002**: 前端开发服务器可以在本地正常访问并展示默认页面
- **SC-003**: 后端开发服务器可以响应基础的健康检查请求
